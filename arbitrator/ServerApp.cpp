...
#include "types/Arbitrator.hpp"
#include "types/ArbitratorPubSubTypes.hpp"
...
{
    SampleInfo info;
    auto request = std::make_shared<OwnershipRequestType>();
...
template<>
Topic* ServerApp::create_topic<OwnershipRequestTypePubSubType>(
        const std::string& topic_name,
        TypeSupport& type)
{
    return detail::create_topic<OwnershipRequestTypePubSubType>(topic_name, participant_, type);
}

template<>
Topic* ServerApp::create_topic<OwnershipReplyTypePubSubType>(
        const std::string& topic_name,
        TypeSupport& type)
{
    return detail::create_topic<OwnershipReplyTypePubSubType>(topic_name, participant_, type);
}

void ServerApp::create_request_entities(
        const std::string& service_name)
{
    request_topic_ = create_topic<OwnershipRequestTypePubSubType>("rq/" + service_name, request_type_);
...
void ServerApp::create_reply_entities(
        const std::string& service_name)
{
    reply_topic_ = create_topic<OwnershipReplyTypePubSubType>("rr/" + service_name, reply_type_);
...
void ServerApp::reply_routine()
{
    while (!is_stopped())
    {
        ...
        if (!is_stopped())
        {
            ...
            if (m_lock_state.compare_exchange_strong(result, true) == true) {
                std::cout << "lock succeed" << std::endl;
                result = true;
            } else {
                std::cout << "lock busy: failure" << std::endl;
                result = false;
            }

            switch(request.request->requester_type()) {
                case InjectionControllerType::POWERHEAD:
                {
                    std::cout << "Powerhead provenance" << std::endl;
                    break;
                }
                case InjectionControllerType::CONSOLE:
                {
                    std::cout << "Console provenance" << std::endl;
                    break;
                }
                case InjectionControllerType::SCANNER_INTERFACE:
                {
                    std::cout << "Scanner interface provenance" << std::endl;
                    break;
                }
                case InjectionControllerType::UNKNOWN:
                default:
                    std::cout << "Unallowed provenance" << std::endl;
                    break;
            }

            std::cout << "Current service strength: " << request.request->current_strength() << std::endl;

            // Prepare the reply
            OwnershipReplyType reply;
            reply.client_id(request.request->client_id());
            reply.ownership_allowed(result);
            reply.strength(20);
