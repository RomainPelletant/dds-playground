...
ClientApp::ClientApp(
    ...
    : request_input_({config.service, config.strength})
        ...
        bool& should_be_ignored)
{
    ...
    if (!should_be_ignored)
    {
        ...
        if (CLIParser::EntityKind::SERVER != entity_kind)
        {
            ...
            //                                              << ": " << remote_participant_guid_prefix);
            request_reply_debug("ClientApp", "Ignoring " << CLIParser::parse_entity_kind(entity_kind)
                                                         << ": " << remote_participant_guid_prefix);
        }
    }
    ...
    if (!should_be_ignored)
    {
        ...
        if (status == rtps::ParticipantDiscoveryStatus::DISCOVERED_PARTICIPANT)
        {
            ...
            request_reply_debug("ClientApp", server_str << ": " << remote_participant_guid_prefix);
        }
            ...
            if (requests_status_.end() != request_status)
            {
                if (!request_status->second)
                {
                    ...
                    request_reply_info("ClientApp", "Reply received from server "
                            ...
                            << "' with ownership allowed: '" << reply.ownership_allowed() << "'"
                            << " 'with strength: '" << reply.strength() << "'");
                }
                else
                {
                    request_reply_debug("ClientApp", "Duplicate reply received from server "
                            << server_guid_prefix << " to request with ID '" << request_status->first.sequence_number()
                            << "' with result: '" << reply.ownership_allowed() << "'"
                            << " 'with strength: '" << reply.strength() << "'");
                    continue;
                }
            }
...
template<>
Topic* ClientApp::create_topic<OwnershipRequestTypePubSubType>(
        const std::string& topic_name,
        TypeSupport& type)
{
...
template<>
Topic* ClientApp::create_topic<OwnershipReplyTypePubSubType>(
        const std::string& topic_name,
        TypeSupport& type)
{
...
{
    OwnershipRequestType request;

    request.client_id(TypeConverter::to_string(participant_->guid().guidPrefix));
    request.requester_type(static_cast<InjectionControllerType>(request_input_.first));
    request.current_strength(request_input_.second);
...
bool ClientApp::send_request(
        const OwnershipRequestType& request)
{
